...
class TestManifest(unittest.TestCase):
    ...
    def test_data_storyteller_toolkit_asset(self) -> None:
        """Validate data-storyteller-s-toolkit asset reconciliation."""
        target = None
        for asset in self.data["assets"]:
            if asset.get("slug") == "data-storyteller-s-toolkit":
                target = asset
                break

        self.assertIsNotNone(target)
        self.assertEqual(target["title"], "Data Storyteller's Toolkit")
        self.assertEqual(target["price"], 11.0)
        self.assertFalse(target["free"])
        self.assertEqual(target["product_id"], "prod_Cw3vVbUJLp3Ip")
