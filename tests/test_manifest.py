...
class TestManifest(unittest.TestCase):
    ...
    def test_social_media_content_calendar_power_pack_asset(self) -> None:
        """Validate social-media-content-calendar-power-pack asset reconciliation."""
        target = None
        for asset in self.data["assets"]:
            if asset.get("slug") == "social-media-content-calendar-power-pack":
                target = asset
                break

        self.assertIsNotNone(target)
        self.assertEqual(target["title"], "Social Media Content Calendar Power Pack")
        self.assertEqual(target["price"], 11.0)
        self.assertFalse(target["free"])
        self.assertEqual(target["product_id"], "prod_a07dGIsKNjDEY")
